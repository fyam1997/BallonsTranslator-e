...
def min_area_rect(points, assume_hull=False):
    ...
    if m == 2:
        ...
        if abs(dx) < 1e-10:  # vertical line
            ...
            angle = (
                180.0 if dx > 0 else 0.0
            )  # cv2 uses 180° for positive horizontal lines
...
def fill_poly(
    image: np.ndarray, pts: Union[Sequence[np.ndarray], np.ndarray], color: int = 1
) -> np.ndarray:
    """
...
def connected_components_with_stats(image: np.ndarray, connectivity: int = 4) -> tuple:
    ...
    if num_labels == 0:
        # Background only
        stats = np.array(
            [[0, 0, image.shape[1], image.shape[0], image.size]], dtype=np.int32
        )
        centroids = np.array(
            [[(image.shape[1] - 1) / 2.0, (image.shape[0] - 1) / 2.0]], dtype=np.float64
        )
    ...
    cen_x = np.divide(
        sums_x, sizes, out=np.zeros_like(sums_x, dtype=float), where=sizes != 0
    )
    cen_y = np.divide(
        sums_y, sizes, out=np.zeros_like(sums_y, dtype=float), where=sizes != 0
    )
...
def line(
    image: np.ndarray, pt1: tuple, pt2: tuple, color: int, thickness: int = 1
) -> np.ndarray:
    """
...
def convert_scale_abs(
    array: np.ndarray, alpha: float = 1.0, beta: float = 0.0
) -> np.ndarray:
    """
...
def threshold(
    array: np.ndarray, thresh: float, maxval: float = 255, thresh_type: int = 0
...
def rectangle(
    ...
    if isinstance(color, int):
        ...
        if len(col) == 3:
            ...
            raise ValueError(
                "Color tuple must have length 1 or 3 for grayscale or RGB images."
            )
    else:
        raise ValueError(
            "Color must be an int or a tuple/list/ndarray of length 1 or 3."
        )
...
def add_weighted(
    ...
    weighted_sum = (
        alpha * src1.astype(np.float64) + beta * src2.astype(np.float64) + gamma
    )
    ...
    output = np.clip(weighted_sum, np.iinfo(src1.dtype).min, np.iinfo(src1.dtype).max)
